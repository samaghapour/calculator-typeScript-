...
class calculator {
    ...
    ;
    appendNumber(number) {
        if (this.current.innerHTML.includes('.') && number == '.')
            return;
    ...
    operation(operationBtn) {
        if (this.current.innerHTML == '')
            return;
        if (operationBtn === '√') {
            this.current.innerHTML = Math.sqrt(parseFloat(this.current.innerHTML));
    ...
    result() {
        ...
        if (isNaN(this.current.innerHTML) || isNaN(this.previous.innerHTML))
            return;
        let compute;
        switch (this.operationSymbol) {
            case '+':
                compute = prevNum + currentNum;
                break;
            case '-':
                compute = prevNum - currentNum;
                break;
...
// variables
const operations = Array.from(document.getElementsByClassName('operation')), numbers = document.querySelectorAll('.numbers'), backspaceBtn = document.getElementById('backspaceBtn'), previousDiv = document.getElementById('previous'), currentDiv = document.getElementById('current'), resultBtn = document.getElementById('result'), clearBtn = document.getElementById('clear'), swithBtn = document.getElementById('swithTheme'), // checkbox for switch theme
calculate = new calculator(previousDiv, currentDiv), switchBox = Array.from(document.getElementsByClassName('switchBox'));
//switch handlers
...
function themeFromLsFunc() {
    if (localStorage.getItem('isDark') == 'true') {
        ...
        switchBox.forEach(item => item.style.color = 'rgba(218, 217, 217, 0.775)');
        ...
        switchBox.forEach(item => item.style.color = 'rgb(96,96,96)');
...
numbers.forEach(number => {
    number.addEventListener('click', (e) => {
        ...
        calculate.appendNumber(target.innerHTML);
    });
});
// backspace listener
...
operations.forEach(operation => {
    operation.addEventListener('click', (e) => {
        const target = e.target;
        calculate.operation(target.innerHTML);
    });
});
